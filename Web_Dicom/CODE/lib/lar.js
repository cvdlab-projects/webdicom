 ...
 * Copyright (c) 2012 Enrico Marino and Federico Spini for CVDLab (https://github.com/cvdlab/)
 * MIT License
 */
 ...
 !(function (exports) {
  ...
  var ccsDot = numeric.ccsDot;
  var ccsDim = numeric.ccsDim;
  ...
  ops.combine = function (matrix_i, matrix_j) {
    if (lar.sparse) {
      return ccsDot(matrix_i, matrix_j);
    } else {
      return dot(matrix_i, matrix_j);
    }
  };
  ...
  ops.extract0 = function (matrix) {
    if (lar.sparse) {
      return ccsDot(ccsSparse(transpose(ccsFull(matrix))), matrix);
    } else {
      return dot(transpose(matrix), matrix);
    }
  };
  ...
  lar.Topology = function (cells) {
    ...
    table[0][n-1] = transpose(binCells_i);

    if (sparse) {
      table[n-1][0] = ccsSparse(table[n-1][0]);
      table[0][n-1] = ccsSparse(table[0][n-1]);
    }

    // loop to generate first row and first col
    for (i = n - 2; i > 0; i -= 1 ) {
      matrix = sparse ? ccsFull(table[i+1][0]) : table[i+1][0];
      binCells_i = binarize(extract(unbinarize(matrix)));
      table[i][0] = binCells_i;
      table[0][i] = transpose(binCells_i);
      if (sparse) {
        table[i][0] = ccsSparse(table[i][0]);
        table[0][i] = ccsSparse(table[0][i]);
      }
    }
  ...
  lar.Topology.prototype.boundary = function (i) {
    ...
    var sparse = lar.sparse;
    var c_ij = this.get(i,i+1);

    c_ij = sparse ? ccsFull(c_ij) : c_ij;

    if (!operator) {
      operator = boundarize(c_ij);
      operator = sparse ? ccsSparse(operator) : operator;
  ...
  lar.Topology.prototype.coboundary = function (i) {
    return ccsSparse(transpose(ccsFull(this.boundary(i))));
  };
  ...
  lar.Model.prototype.getChain = function (d, selector, included) {
    ...
    var length = sparse ? ccsDim(c_d0)[0] : c_d0.length;
    var chain = included ? chainPut1(length, selector) : chainPut0(length, selector);

    c_d0 = sparse ? ccsFull(c_d0) : c_d0;
  ...
  lar.Model.prototype.map = function (i, j, chain, included) {
    ...
    var length = sparse ? ccsDim(c_ij)[0] : c_ij.length;
    var explicitChain = included ? chainPut1(length, chain) : chainPut0(length, chain);

    if (! sparse) {
      return dot(c_ij, explicitChain);
    } else {
      explicitChain = explicitChain.map(function (val) {
        return [val];
      });
      return flat(ccsDot(c_ij,ccsSparse(explicitChain)));
    }
  };
  ...
  lar.Model.prototype.boundary = function (p, chain, included) {
    ...
    var c_p0 = topology.get(p,0);
    var length = sparse ? ccsDim(c_p0)[0] : c_p0.length;
    var boundary;
    var cells;
    var explicitChain;
    ...
    if (!sparse) {
      boundary = dot(operator, explicitChain);
    } else {
      c_i0 = ccsFull(c_i0);
      explicitChain = explicitChain.map(function (val) {
        return [val];
      });
      boundary = ccsFull(ccsDot(operator, ccsSparse(explicitChain)));
  ...
  lar.Model.prototype.coboundary = function (p, chain, included) {
    ...
    if (!sparse) {
      coboundary = dot(operator, explicitChain);
    } else {
      c_i0 = ccsFull(c_i0);
      explicitChain = explicitChain.map(function (val) {
        return [val];
      });
      coboundary = ccsFull(ccsDot(operator, ccsSparse(explicitChain)));
      coboundary = flat(coboundary);
    }
